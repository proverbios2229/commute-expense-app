...
　　・交通費申請は、日付を1日だけでなく複数日を選択して一括申請できる。
　　・日付選択はカレンダーUIで行い、ユーザーは複数日を選択/解除できる。
　　・カレンダー下部の「確定」操作により、選択日がフォームに確定される。
　　・確定された日付は、フォーム上で**複数日を一覧（チップ表示など）**として確認・削除できる。
　　・一括申請では、選択された各日付に対して同一条件（区間・往復・備考）で申請が作成される。
...
　　・一括申請の最大選択日数は、大量登録による誤操作・サーバ負荷を防ぐため 31日 とする（MVP）。
　　・同一ユーザーの同一日付に対して既に申請が存在する場合の扱い：エラーとして登録しない（MVP）。
※将来、スキップや上書きに拡張可能。
　　・一括申請は、処理途中で一部のみ登録されることを防ぐため、 全件成功 or 全件失敗（トランザクション） とする（MVP）。
　2.利用環境
